...
namespace Signicat.DigitalEvidenceManagement.Entities
{
    public class DemRecord
    {
        ...
        public RecordTypes? RecordType {
            get
            {
                RecordTypes recordType = RecordTypes.OTHER;
                if (Metadata?.Custom is not null && Metadata.System.ContainsKey("type") && 
                    Enum.TryParse<RecordTypes>(Metadata.System["type"].ToString(), out recordType))
                {
                    return recordType;
                }
        ...
        /// </summary>
        public DemRecordMetadata Metadata { get; set; } = new DemRecordMetadata();
